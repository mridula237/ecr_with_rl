...
def evaluate_model(model, 
                   ...
                   test_loader, 
                   epoch, 
                   include_audio=False,  # This parameter is not used in the function but can be used to conditionally include audio processing
...
def train_one_epoch(
    ...
    for batch in tqdm(train_loader, desc=f"Training Epoch {epoch + 1}"):
        ...
        if include_audio:
            # If audio_embed is not None, move it to the device
            inputs["audio_inputs"] = audio_embed.to(device) if audio_embed is not None else None #TODO: review this step
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        if unfreeze_on_epoch is not None and epoch == unfreeze_on_epoch - 1:
            ...
            #include recently unfreezed paramaters in the optimizer
            ...
            if include_audio:
                for p in model.wav_model.feature_extractor.parameters():
                    p.requires_grad = True
                # include recently unfreezed audio parameters in the optimizer
